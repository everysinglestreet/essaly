...
export const executeTilemaker = async ({ request }: { request: Request }): Promise<EssalyResponse> => {
  try {
    ...
    const { input, output } = request.body as ExecuteTilemakerInput;
    const res = await execute({ command: `cd ${ESS_TASKING_PATH} && TILEMAKER_INPUT=${input} TILEMAKER_OUTPUT=${output} task tilemaker:execute` });
